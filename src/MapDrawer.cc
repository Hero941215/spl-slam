...
void MapDrawer::DrawMapLines()
{
    ...
    for(size_t i=0, iend=vpMLs.size(); i<iend;i++)
    {
        ...
        if(vpMLs[i]->isBad() || spRefMLs.count(vpMLs[i]))
            continue;
        cv::Mat posFir = vpMLs[i]->GetFirPWorldPos();
	cv::Mat posEnd = vpMLs[i]->GetEndPWorldPos();
	...
	glVertex3f(posFir.at<float>(0),posFir.at<float>(1),posFir.at<float>(2));
    ...
    for(set<MapLine*>::iterator sit=spRefMLs.begin(), send=spRefMLs.end(); sit!=send; sit++)
    {
        ...
        cv::Mat posFir = (*sit)->GetFirPWorldPos();
	cv::Mat posEnd = (*sit)->GetEndPWorldPos();
